...
export const $header = (context, entity, headerNameOrOptions) => {
    ...
    if (headerNameOrOptions) {
        if (typeof headerNameOrOptions === "string") {
            ...
            if (format) {
                reportDeprecated(context.program, "The `format` option of `@header` decorator is deprecated. Use `explode: true` instead of `form` and `multi`. `csv` or `simple` is the default now.", entity);
                if (format === "csv" ||
                    format === "tsv" ||
                    format === "pipes" ||
                    format === "ssv" ||
