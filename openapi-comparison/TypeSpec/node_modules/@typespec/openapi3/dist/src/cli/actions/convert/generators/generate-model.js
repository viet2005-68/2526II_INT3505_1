...
function generateUnion(union, context) {
    ...
    const getVariantName = (member) => {
        ...
        const value = memberSchema.properties?.[union.schema.discriminator.propertyName]
            ?.enum?.[0];
        return value ? `${value}: ` : "";
    };
    if (schema.enum) {
        ...
        definitions.push(...schema.oneOf.map((member) => getVariantName(member) + context.generateTypeFromRefableSchema(member, union.scope)));
    }
