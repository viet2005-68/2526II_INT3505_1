...
import { getDirectoryPath, getRelativePathFromDirectory, joinPaths, resolvePath, } from "./path-utils.js";
...
export async function compile(host, mainFile, options = {}, oldProgram) {
    ...
    const { program, shouldAbort } = await logger.trackAction("Compiling...", "Compiling", () => createProgram(host, mainFile, options, oldProgram));
    if (shouldAbort) {
        return program;
    }
    // Emitter stage
        ...
        for (const set of symbols.duplicates.values()) {
            for (const symbol of set) {
                if (!duplicateSymbols.has(symbol)) {
                    ...
                    const name = symbol.flags & 32768 /* SymbolFlags.Using */ ? symbol.symbolSource.name : symbol.name;
                    reportDiagnostic(createDiagnostic({
...
async function emit(emitter, program, options = {}) {
    ...
    await logger.trackAction(`Running ${emitterName}...`, `${emitterName}\t${relativePathForEmittedFiles}`, () => runEmitter(emitter, program));
}
/**
...
function logEmittedFilesPath(projectRoot, emitterOutputDir) {
